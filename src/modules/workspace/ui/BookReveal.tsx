 ...
 * here is what the page remembers. The options share the strip's width evenly,
 * so the whole set is reachable without a sideways scroll and each one is as
 * wide as the others; a long name is truncated rather than pushing its
 * neighbours around.
 *
        ...
        >
          {options.map((option) => (
            <button
              ...
              className={cn(
                "min-w-0 flex-1 rounded-md px-3 py-1.5 text-xs font-medium transition-colors duration-[var(--motion-feedback)]",
                scope === option.scope
