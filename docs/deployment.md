...
迁移链已经压缩成 `0000_baseline.sql`，它等于 0057 之前所有迁移执行完后的 schema。
还没升到 0057 的数据库不能直接升级到这之后的版本：`npm run db:migrate` 会拒绝执行，
并提示先部署 `pre-baseline` 这个 Git tag，让它把完整的旧迁移链跑完，然后再部署新版本。

迁移在新版本构建之前执行，而旧版本在新构建上线之前（以及构建失败时）会继续对外服务。
因此每个迁移都必须同时兼容正在运行的旧版本：删列、改名这类改动要分成先停止读写、
再在下一次部署里删除两步。
