...
describe("SourceDocumentCameraPanel", () => {
  it("says why a browser with no camera API cannot take a photo", () => {
    ...
    expect(screen.getByText(sourceDocumentInputCopy.cameraUnsupported)).toBeInTheDocument();
    expect(view.queryByRole("button", { name: sourceDocumentInputCopy.capturePhoto })).toBeNull();
  });

  it("blames the address when the page is not a secure context", () => {
    const { view } = renderPanel({ status: "insecure" });

    expect(screen.getByText(sourceDocumentInputCopy.cameraInsecure)).toBeInTheDocument();
    // Offering to reopen a camera that the browser will refuse would be a lie.
    expect(view.queryByRole("button", { name: sourceDocumentInputCopy.openCamera })).toBeNull();
  });

  it("asks before trying the camera again after a refusal", () => {
    const { onRetry } = renderPanel({ status: "unavailable" });

    expect(screen.getByText(sourceDocumentInputCopy.cameraUnavailable)).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: commonCopy.retry }));
  ...
  it("takes a photo from a running viewfinder", () => {
    ...
    const shutter = screen.getByRole("button", { name: sourceDocumentInputCopy.capturePhoto });
  ...
  it("holds the shutter back while the camera is still starting", () => {
    ...
    expect(
      screen.getByRole("button", { name: sourceDocumentInputCopy.capturePhoto })
    ).toBeDisabled();
    expect(screen.getByText(sourceDocumentInputCopy.cameraStarting)).toBeInTheDocument();
  });
  ...
  it("stops offering the shutter once the images are full", () => {
    ...
    expect(shutter.parentElement).toHaveAttribute(
      "title",
      sourceDocumentInputCopy.tooManyImages({ count: MAX_FILES })
    );
    fireEvent.click(shutter);
  ...
  it("offers the front/back switch only when the device has more than one camera", () => {
    ...
    expect(
      single.view.queryByRole("button", { name: sourceDocumentInputCopy.switchCamera })
    ).toBeNull();

    const { onSwitchFacing } = renderPanel({ canSwitch: true });
    fireEvent.click(screen.getByRole("button", { name: sourceDocumentInputCopy.switchCamera }));
  ...
  it("collapses to one button that brings the camera back", () => {
    ...
    expect(view.queryByRole("button", { name: sourceDocumentInputCopy.capturePhoto })).toBeNull();
    fireEvent.click(screen.getByRole("button", { name: sourceDocumentInputCopy.openCamera }));
  ...
  it("collapses from the viewfinder itself", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: sourceDocumentInputCopy.collapseCamera }));
